 ...
 * 
 */
public class KLineDraw_SurfaceView extends SurfaceView implements Callback, Runnable {
	...
	public KLineDraw_SurfaceView(Context context, float wid, float hei, String ex) {
		super(context);
	...
	int nextRed1 = Color.argb(255, 100, 100, 100);
	...
	public void draw2() {
		...
		try {// 第一种方法防止退出时异常: 当isThreadRunning为false时, 最后还是会执行一次drawView方法,
			...
			if (surfaceHolder != null) {
				...
				topWhite = DensityUtil.dip2px(context, 15);
				kHeight = (maxH - topWhite) * 0.7f;
				tradeHeight = maxH - kHeight - topWhite;
				tableWidth = width - rightWhite - leftWhite;

				// Log.i("temp", "height->" + height + ",maxH->" + maxH +
				// ",kHeight->" + kHeight + ",width" + width);
				...
				if (false) {
					...
					// Log.i("temp", "k_sf_Thread.run.draw");
					// 初始化Draw数组 完整的
					count_opentouch2--;
			...
			if (canvas != null) {
				// 3. 将画布解锁并显示在屏幕上
				// Log.i("draw", "finally-unlockCanvasAndPost");
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
		// log.i("draw", "surfaceChanged");
	}

	public void surfaceCreated(SurfaceHolder holder) {
		// 创建surfaceView时启动线程
		// log.i("draw", "surfaceCreated");
		new Thread(this).start();
	}

	public void surfaceDestroyed(SurfaceHolder holder) {
		// 当surfaceView销毁时, 停止线程的运行. 避免surfaceView销毁了线程还在运行而报错.
		// log.i("draw", "surfaceDestroyed");
